...
Current state: playable `v0.0.93` proof of concept with automated Windows build/test matrix.
...
- Packaged Bellows Node smoke test.
...
- `SteamworksAudio` now generates and loops a low-volume brassworks machinery ambience bed.
- Ambience volume follows the existing master-volume setting.
- Level validation and runtime smoke verify the ambience configuration and active packaged loop.
...
- Steam Scattergun now has its own first-person brass/walnut triple-barrel viewmodel.
- Weapon switching activates/deactivates the pistol and scattergun viewmodels at runtime.
- Weapon-switch smoke verifies scattergun view activation and pistol reactivation.

## What v0.0.76 Adds

- Steam Scattergun firing now spawns dedicated pressure-ring, steam-core, and brass-spark blast VFX.
- Weapon-switch smoke verifies the scattergun blast VFX spawns with visible pieces.

## What v0.0.77 Adds

- Steam Scattergun now has a dedicated procedural low-pressure blast audio cue.
- Runtime smoke verifies the scattergun cue is configured, and weapon-switch smoke verifies scattergun firing routes through it.

## What v0.0.78 Adds

- Level03 now includes a stationary Bellows Node support-machine prototype.
- Bellows Node emits a damaging pressure pulse with dedicated pulse VFX and can be destroyed.
- Full matrix includes packaged `V0_BELLOWS_NODE_PASS` coverage.

## What v0.0.79 Adds

- Bellows Node pressure pulses now over-pressurize nearby Scrappers with a short speed boost.
- Bellows Node smoke verifies pulse damage, pulse VFX, nearby Scrapper boost, durability, and destruction VFX.

## What v0.0.80 Adds

- Boosted Scrappers now show animated brass/steam pressure-boost VFX while over-pressurized.
- Bellows Node smoke verifies the boost-state VFX appears with enough visible pieces before the destruction check.

## What v0.0.81 Adds

- Bellows Node pulses now route through a dedicated procedural bellows/steam audio cue.
- Runtime smoke verifies the support cue exists, and Bellows Node smoke verifies the spatial pulse cue route.

## What v0.0.82 Adds

- Steam Scattergun acquisition smoke now routes through the real Level03 world pickup instead of directly granting the weapon.
- Weapon pickups now spawn a dedicated brass/steam weapon-acquisition VFX burst.
- Weapon-switch smoke verifies the real pickup unlock/equip path plus pickup VFX visibility before firing tests.

## What v0.0.83 Adds

- Weapon pickups now have a dedicated procedural steamwork acquisition audio cue.
- The Steam Scattergun pickup definition uses the new cue instead of the generic ammo pickup tick.
- Runtime and weapon-switch smoke verify the cue exists and routes through the real Level03 pickup.

## What v0.0.84 Adds

- Steam Scattergun right-mouse slug now has its own procedural audio cue.
- Steam Scattergun slug fire now spawns a dedicated pressure-spear VFX.
- Weapon-switch smoke verifies slug ammo cost, cue routing, VFX spawn, non-lethal slug damage, then primary-fire kill.

## What v0.0.85 Adds

- Steam Scattergun world pickup now has a richer steampunk display stand, iron yoke, enamel nameplate, brass top rib, walnut pump grip, pressure coil, rear valve wheel, and shell rack.
- Level03 validation now requires the named pickup-polish pieces so the breaching weapon silhouette stays intact after scene regeneration.
- Full V0 matrix produced `Builds/Windows/v0.0.85/BrassworksBreach_v0.0.85.exe`.

## What v0.0.86 Adds

- Level03 Steam Scattergun pickup now has brass route strips, floor chevrons, pressure piping, lamps, and a `BREACH TOOL` world label.
- Level03 validation now requires the named pickup readability cues.
- Full V0 matrix produced `Builds/Windows/v0.0.86/BrassworksBreach_v0.0.86.exe`.

## What v0.0.87 Adds

- Pressure Pistol right-mouse Pressure Burst now routes through a dedicated procedural pressure-dump audio cue.
- Pressure Burst now spawns its own pressure-ring, steam-core, brass-valve, and shard VFX instead of relying on generic pistol feedback.
- Runtime and combat-scenario smoke verify Pressure Burst cue configuration, audio routing, VFX spawn, ammo accounting, and non-lethal setup before primary-shot checks.

## What v0.0.88 Adds

- Pressure Pistol secondary fire now drives distinct viewmodel pressure-dump motion.
...
- Scrapper death now uses a richer shutdown variant with boiler cap, chest plate, cutter shards, flywheel gears, valve wheel, pressure tank burst, furnace flash, steam puffs, brass sparks, and pressure ring.
- Generic machine death VFX still serves other machine roles, while Scrappers get a dedicated shutdown style.
- Combat smoke now verifies the Scrapper-specific shutdown detail before passing.
- Batch 02 view-only room/object render JPGs were added under `Documentation/ConceptRenders/`; start with `CONTACTSHEET_batch02_modular_room_object_renders.jpg`.
- Full V0 matrix produced `Builds/Windows/v0.0.90/BrassworksBreach_v0.0.90.exe`.

## What v0.0.91 Adds

- Lancer windup now spawns a valve-rifle charge VFX state before pressure bolts fire.
- Lancer windup now plays a dedicated `LancerFireTell` procedural charge audio cue.
- Level validation requires configured Lancer fire-tell components on generated Lancers.
- Runtime smoke verifies the new audio cue, and ranged-combat smoke verifies Lancer fire-tell VFX/audio before pressure-bolt VFX and damage.
- Curie completed the first high-fidelity non-shipping lookdev package and review renders; start with `Documentation/ConceptRenders/CONTACTSHEET_LOOKDEV_HFLD_Batch01_nonshipping.jpg`.
- Full V0 matrix produced `Builds/Windows/v0.0.91/BrassworksBreach_v0.0.91.exe`.

## What v0.0.92 Adds

- Pressure bolts now use swept collision checks and spawn a dedicated impact VFX when they hit the player or world.
- `PressureBoltImpactVfx` adds a pressure flash core, expanding rings, steam pop, and brass/orange shards.
- Ranged-combat smoke now requires the Lancer bolt impact VFX when bolt damage lands.
- High-fidelity lookdev recovery is now narrowed to the Pressure Pistol first; the earlier broad Batch01 render is treated as rejected planning/reference, not accepted art direction.
- Full V0 matrix produced `Builds/Windows/v0.0.92/BrassworksBreach_v0.0.92.exe`.

## What v0.0.93 Adds

- Bulwark hammer windup now spawns a dedicated slam warning VFX state before damage lands.
- Bulwark windup now plays a dedicated `BulwarkAttackTell` procedural audio cue.
- Level validation requires configured Bulwark attack-tell components on generated Bulwarks.
- Bulwark-combat smoke verifies the tell VFX/audio appears before slam damage, then verifies heavy durability, hit VFX, and death VFX.
- Full V0 matrix produced `Builds/Windows/v0.0.93/BrassworksBreach_v0.0.93.exe`.
...
- `Documentation/PARALLEL_WORKSTREAM_STATUS.md`
