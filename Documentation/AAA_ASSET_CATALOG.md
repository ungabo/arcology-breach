...
| RENDER-003 | High-fidelity lookdev recovery | P1 | active | Batch01 and Recovery03 were rejected visually. Current active work is Unity-only component-first lookdev with temporary editor-created scenes and RenderTexture/PNG output under `Documentation/ConceptRenders/`. Banach owns pressure-pistol component refinement and Turing-Corridor owns corridor/vault-door lookdev. External renderer lanes are superseded unless explicitly reopened later. |
...
| UI-007 | Settings and accessibility | P2 | verified | Mouse sensitivity, master volume, flash intensity, resolution preset, and fullscreen controls exist on main and pause menus. Flash intensity persists through `GameSettings` and scales damage flash/player damage VFX as of `v0.1.0`; resolution/fullscreen persist and are smoke-tested as of `v0.1.9`; color readability remains planned. |
