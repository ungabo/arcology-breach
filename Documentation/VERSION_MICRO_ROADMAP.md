...
| Version | Planned Slice | Intent | Expected Verification |
| --- | --- | --- | --- |
| v0.1.30 | Service lift call box asset promotion | Promote `ServiceLiftCallBoxPrototype` as route-safe wall-adjacent lift/hoist control dressing across current level transitions, using the v0.1.30 asset-queue packet. | Full V0 matrix with validator coverage for named hierarchy, material roles, detail counts, and no-collider route safety. |
| v0.1.31 | Gear key plinth asset promotion | Promote `GearKeyPlinthPrototype` around existing key/objective pickup presentation if v0.1.30 call boxes validate cleanly. | Full V0 matrix with pickup reachability, named hierarchy, material-role, and no-collider validation. |
| v0.1.32 | Route/distribution hardening follow-up | Convert any new accepted manual notes into tracked work; otherwise continue small release or route-readability hardening. | Route audit plus full V0 matrix if code, scene, package, or generated evidence changes. |
