...
namespace BrassworksBreach.EnemyAnimationProxySet01.Editor
{
    public static class EnemyAnimationProxySet01Generator
    {
        private const string PackageName = "com.brassworks.sidecar.enemy-animation-proxy-set01";
        ...
        private const string ClipRoot = PackageRoot + "/Runtime/AnimationClips";
        private const string MetadataRoot = PackageRoot + "/Runtime/Metadata";
        private const string RenderDocFolder = "Documentation/ConceptRenders/V0_1_44_EnemyAnimationProxySet01";
        private const string ProductionDocFolder = "Documentation/AssetProduction/V0_1_44_EnemyAnimationProxySet01";
        ...
        {
            new CandidateSpec("EAP01_ScrapperAshcan_01_IdleBrace", "Scrapper / Ashcan", "Idle Brace", 0, 1.48f, 1.02f, 0.92f, "low idle proxy with saw weight offset"),
            new CandidateSpec("EAP01_ScrapperAshcan_02_WindupCoil", "Scrapper / Ashcan", "Windup Coil", 1, 1.55f, 1.18f, 1.02f, "wide windup proxy with exposed pressure mass"),
            new CandidateSpec("EAP01_ScrapperAshcan_03_SawLunge", "Scrapper / Ashcan", "Saw Lunge", 2, 1.62f, 1.20f, 1.20f, "forward attack proxy with jaw saw extension"),
            new CandidateSpec("EAP01_ScrapperAshcan_04_RecoverDrag", "Scrapper / Ashcan", "Recover Drag", 1, 1.38f, 1.28f, 1.04f, "low recovery proxy with dragging blade read"),
            new CandidateSpec("EAP01_LancerPressureSpindle_01_AimLine", "Lancer / Pressure Spindle", "Aim Line", 0, 1.92f, 0.74f, 1.34f, "narrow aiming proxy with readable lance axis"),
            new CandidateSpec("EAP01_LancerPressureSpindle_02_ChargeStep", "Lancer / Pressure Spindle", "Charge Step", 1, 2.04f, 0.82f, 1.44f, "charged stride proxy with backpack coils"),
            new CandidateSpec("EAP01_LancerPressureSpindle_03_ThrustPeak", "Lancer / Pressure Spindle", "Thrust Peak", 2, 1.88f, 0.90f, 1.54f, "paired drill thrust proxy with cyan pressure bridge"),
            new CandidateSpec("EAP01_LancerPressureSpindle_04_RecoilVent", "Lancer / Pressure Spindle", "Recoil Vent", 1, 1.98f, 0.88f, 1.20f, "post-shot recoil proxy with rear vent emphasis"),
            new CandidateSpec("EAP01_BulwarkGatehammer_01_GuardSet", "Bulwark / Gatehammer", "Guard Set", 0, 2.10f, 1.52f, 1.00f, "wide shield wall proxy with furnace weakpoint"),
            new CandidateSpec("EAP01_BulwarkGatehammer_02_HammerRaise", "Bulwark / Gatehammer", "Hammer Raise", 1, 2.25f, 1.72f, 1.04f, "raised hammer anticipation proxy"),
            new CandidateSpec("EAP01_BulwarkGatehammer_03_SlamImpact", "Bulwark / Gatehammer", "Slam Impact", 2, 2.00f, 1.64f, 1.20f, "stocky slam contact proxy with hot braces"),
            new CandidateSpec("EAP01_BulwarkGatehammer_04_StaggerOpen", "Bulwark / Gatehammer", "Stagger Open", 1, 2.12f, 1.78f, 1.12f, "opened guard proxy exposing furnace center"),
            new CandidateSpec("EAP01_WardenGovernor_01_CommandIdle", "Warden / Governor", "Command Idle", 0, 2.48f, 1.28f, 1.12f, "elite tower command proxy with gear halo"),
            new CandidateSpec("EAP01_WardenGovernor_02_SignalRaise", "Warden / Governor", "Signal Raise", 1, 2.72f, 1.54f, 1.26f, "signal raise proxy with overseer bell silhouette"),
            new CandidateSpec("EAP01_WardenGovernor_03_OrderPulse", "Warden / Governor", "Order Pulse", 2, 2.64f, 1.60f, 1.28f, "order pulse proxy with lamps and command fins"),
            new CandidateSpec("EAP01_WardenGovernor_04_ShutdownDrop", "Warden / Governor", "Shutdown Drop", 1, 2.28f, 1.44f, 1.18f, "collapsed authority proxy with lowered command center")
        };

        [MenuItem("Brassworks Breach/Sidecars/Enemy Animation Proxy Set 01/Generate Package")]
        public static void GenerateVisualSet()
        {
            ...
            CreatePlaceholderAnimationClips();
            WriteRuntimeCatalog();
            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();
            Debug.Log("EAP01 v0.1.44 generated: " + Candidates.Length + " prefabs, 15 materials, 8 reusable meshes, 4 placeholder clips, and a runtime catalog.");
        }

        [MenuItem("Brassworks Breach/Sidecars/Enemy Animation Proxy Set 01/Render Preview PNGs")]
        public static void RenderPreviewPngs()
        {
            ...
            foreach (var existingPreview in Directory.GetFiles(outputRoot, "EAP01_*.png"))
            {
                File.Delete(existingPreview);
            }
            ...
            foreach (var family in Candidates.Select(candidate => candidate.Family).Distinct())
            {
                ...
                var fileName = "EAP01_v0.1.44_" + MakeSafeFileName(family) + "_contact_sheet.png";
                RenderContactSheet(outputRoot, fileName, familyCandidates, Math.Min(3, familyCandidates.Length), 1800, 1200);
            }

            foreach (var candidate in Candidates.GroupBy(candidate => candidate.Family).SelectMany(group => group.Take(3)))
            {
                RenderSingleCandidate(outputRoot, candidate, 1400, 1400);
            }

            AssetDatabase.Refresh();
            Debug.Log("EAP01 v0.1.44 preview PNGs rendered to: " + outputRoot);
        }
        ...
        private static Dictionary<string, Material> CreateMaterials()
        {
            ...
            {
                ["Brass"] = UpsertMaterial("EAP01_MAT_AgedBrass", AgedBrass, 0.86f, 0.38f, Color.black, false),
                ["Iron"] = UpsertMaterial("EAP01_MAT_BlackenedIron", BlackenedIron, 0.92f, 0.24f, Color.black, false),
                ["Copper"] = UpsertMaterial("EAP01_MAT_BurnishedCopper", BurnishedCopper, 0.78f, 0.34f, Color.black, false),
                ["Leather"] = UpsertMaterial("EAP01_MAT_OilyBellowsLeather", BellowsLeather, 0.0f, 0.46f, Color.black, false),
                ["Glass"] = UpsertMaterial("EAP01_MAT_GrimyGlassLens", GrimyGlass, 0.0f, 0.80f, Color.black, true),
                ["Amber"] = UpsertMaterial("EAP01_MAT_AmberWarningLamp", AmberLamp, 0.0f, 0.38f, AmberLamp * 2.8f, false),
                ["Cyan"] = UpsertMaterial("EAP01_MAT_CyanPressurePilot", CyanPilot, 0.0f, 0.34f, CyanPilot * 2.4f, false),
                ["Furnace"] = UpsertMaterial("EAP01_MAT_FurnaceOrangeGlow", FurnaceOrange, 0.0f, 0.30f, FurnaceOrange * 3.0f, false),
                ["Soot"] = UpsertMaterial("EAP01_MAT_SootBlack", SootBlack, 0.30f, 0.16f, Color.black, false),
                ["Hazard"] = UpsertMaterial("EAP01_MAT_ChippedHazardPaint", HazardPaint, 0.16f, 0.30f, Color.black, false),
                ["Rivet"] = UpsertMaterial("EAP01_MAT_PolishedRivetWear", PolishedRivet, 1.0f, 0.52f, Color.black, false),
                ["Ceramic"] = UpsertMaterial("EAP01_MAT_BoilerCeramic", BoilerCeramic, 0.0f, 0.25f, Color.black, false),
                ["HotIron"] = UpsertMaterial("EAP01_MAT_RedHotIronSlit", RedHotIron, 0.35f, 0.28f, RedHotIron * 2.6f, false),
                ["Patina"] = UpsertMaterial("EAP01_MAT_VerdigrisPressurePipe", VerdigrisPipe, 0.72f, 0.38f, Color.black, false),
                ["Ghost"] = UpsertMaterial("EAP01_MAT_GhostedSilhouetteEnvelope", GhostedReadability, 0.0f, 0.18f, Color.black, true)
            };
        }
        ...
        private static Dictionary<string, Mesh> CreateMeshes()
        {
            ...
            {
                ["SawBlade"] = UpsertMesh("EAP01_MESH_28ToothSawBlade", CreateSawBladeMesh("EAP01_MESH_28ToothSawBlade", 28, 0.22f, 0.40f)),
                ["LanceTip"] = UpsertMesh("EAP01_MESH_RivetLanceTip", CreateConeMesh("EAP01_MESH_RivetLanceTip", 0.18f, 0.78f, 20)),
                ["ShieldPlate"] = UpsertMesh("EAP01_MESH_BulwarkShieldPlate", CreateShieldPlateMesh("EAP01_MESH_BulwarkShieldPlate")),
                ["BellowsRib"] = UpsertMesh("EAP01_MESH_BellowsRib", CreateBellowsRibMesh("EAP01_MESH_BellowsRib")),
                ["GearHalo"] = UpsertMesh("EAP01_MESH_CommandGearHalo", CreateGearRingMesh("EAP01_MESH_CommandGearHalo", 24, 0.30f, 0.48f)),
                ["OverseerFin"] = UpsertMesh("EAP01_MESH_OverseerShoulderFin", CreateFinMesh("EAP01_MESH_OverseerShoulderFin")),
                ["GaugeFace"] = UpsertMesh("EAP01_MESH_PressureGaugeFace", CreateGearRingMesh("EAP01_MESH_PressureGaugeFace", 18, 0.035f, 0.20f)),
                ["DrillBit"] = UpsertMesh("EAP01_MESH_TwinnedDrillBit", CreateDrillBitMesh("EAP01_MESH_TwinnedDrillBit", 0.16f, 0.62f, 10))
            };
        }
        ...
        private static GameObject BuildCandidate(CandidateSpec candidate, Dictionary<string, Material> mat, Dictionary<string, Mesh> mesh)
        {
            ...
            if (candidate.Family.StartsWith("Scrapper", StringComparison.Ordinal))
            {
                BuildSawScrapper(candidate, groups, mat, mesh);
            }
            else if (candidate.Family.StartsWith("Lancer", StringComparison.Ordinal))
            {
                BuildRivetLancer(candidate, groups, mat, mesh);
            }
            else if (candidate.Family.StartsWith("Bulwark", StringComparison.Ordinal))
            {
                BuildBulwarkFurnace(candidate, groups, mat, mesh);
            }
            else if (candidate.Family.StartsWith("Warden", StringComparison.Ordinal))
            {
        ...
        private static GameObject CreateBaseCandidate(CandidateSpec candidate)
        {
            ...
            var sockets = CreateEmpty("sockets_visual_markers", root.transform);
            CreateSocket("socket_head_look", sockets.transform, new Vector3(0f, candidate.Height * 0.86f, -0.34f));
            CreateSocket("socket_primary_tool_tip", sockets.transform, new Vector3(0f, candidate.Height * 0.50f, -candidate.Depth * 0.72f));
            CreateSocket("socket_left_hand_proxy", sockets.transform, new Vector3(-candidate.Width * 0.42f, candidate.Height * 0.48f, -0.18f));
            CreateSocket("socket_right_hand_proxy", sockets.transform, new Vector3(candidate.Width * 0.42f, candidate.Height * 0.48f, -0.18f));
            CreateSocket("socket_back_steam_vent", sockets.transform, new Vector3(0f, candidate.Height * 0.78f, candidate.Depth * 0.30f));
            CreateSocket("socket_core_readability", sockets.transform, new Vector3(0f, candidate.Height * 0.58f, -0.42f));
            return root;
        }
        ...
        private static void CreatePlaceholderAnimationClips()
        {
            var clipIds = new[]
            {
                "EAP01_CLIP_ScrapperAshcan_PoseProxyOnly",
                "EAP01_CLIP_LancerPressureSpindle_PoseProxyOnly",
                "EAP01_CLIP_BulwarkGatehammer_PoseProxyOnly",
                "EAP01_CLIP_WardenGovernor_PoseProxyOnly"
            };

            foreach (var clipId in clipIds)
            {
                var path = ClipRoot + "/" + clipId + ".anim";
        ...
        {
            var catalogPath = ResolvePackageAssetPath(MetadataRoot + "/EAP01_RuntimeCatalog_v0.1.44.json");
            Directory.CreateDirectory(Path.GetDirectoryName(catalogPath));

            var json = "{\n" +
                "  \"catalog_id\": \"EAP01_RuntimeCatalog_v0.1.44\",\n" +
                "  \"package_name\": \"" + PackageName + "\",\n" +
                "  \"visual_only\": true,\n" +
                "  \"gameplay_authority\": false,\n" +
                "  \"forbidden_components\": [\"Collider\", \"Rigidbody\", \"NavMeshAgent\", \"AudioSource\", \"Damage\", \"Pickup\", \"GameplayController\"],\n" +
                "  \"families\": [\"Scrapper / Ashcan\", \"Lancer / Pressure Spindle\", \"Bulwark / Gatehammer\", \"Warden / Governor\"],\n" +
                "  \"prefab_count\": " + Candidates.Length + ",\n" +
                "  \"material_count\": 15,\n" +
                "  \"mesh_count\": 8,\n" +
                "  \"socket_markers\": [\"socket_head_look\", \"socket_primary_tool_tip\", \"socket_left_hand_proxy\", \"socket_right_hand_proxy\", \"socket_back_steam_vent\", \"socket_core_readability\"],\n" +
                "  \"prefabs\": [\n" +
                string.Join(",\n", Candidates.Select(candidate =>
                    "    { \"id\": \"" + candidate.Id + "\", \"family\": \"" + candidate.Family + "\", \"variant\": \"" + candidate.Variant + "\", \"path\": \"" + PrefabRoot + "/" + candidate.Id + ".prefab\" }")) +
                "\n  ],\n" +
                "  \"notes\": [\n" +
                "    \"Proxy assets are Unity-generated visual planning assets only.\",\n" +
                "    \"AnimationClip assets are placeholder timing labels and are not authoritative gameplay animation.\",\n" +
                "    \"Prefabs intentionally omit colliders, rigidbodies, nav agents, gameplay controllers, damage, pickups, and autonomous audio.\"\n" +
                "  ]\n" +
                "}\n";

            File.WriteAllText(catalogPath, json);
            AssetDatabase.ImportAsset(MetadataRoot + "/EAP01_RuntimeCatalog_v0.1.44.json");
        }
        ...
        private static GameObject CreateSocket(string name, Transform parent, Vector3 localPosition)
        {
        ...
        private static void RenderSingleCandidate(string outputRoot, CandidateSpec candidate, int width, int height)
        {
            ...
            var camera = CreatePreviewCamera(candidate.Family.StartsWith("Warden", StringComparison.Ordinal) ? 42f : 38f);
            FrameBounds(camera, CalculateBounds(new[] { instance }), 1.30f, (float)width / height);
            RenderCameraToPng(camera, Path.Combine(outputRoot, candidate.Id + "_v0.1.44.png"), width, height);
        }
        ...
        private static int WriteUnityValidationReport()
        {
            ...
            if (pngCount < 8 || pngCount > 16) errors++;
            if (colliderCount != 0) errors++;

            var reportRoot = ResolveRepoRelativeFolder(ProductionDocFolder);
            Directory.CreateDirectory(reportRoot);
            var reportPath = Path.Combine(reportRoot, "unity_validation_report_v0.1.44.json");
            var json = "{\n" +
                ...
                "  \"expected_meshes\": 8,\n" +
                "  \"expected_preview_pngs_minimum\": 8,\n" +
                "  \"expected_preview_pngs_maximum\": 16\n" +
                "}\n";
            File.WriteAllText(reportPath, json);
            Debug.Log("EAP01_UNITY_VALIDATION_" + (errors == 0 ? "PASS" : "FAIL") + " report=" + reportPath);
            return errors;
        }
        ...
        private static void EnsurePackageFolders()
        {
            ...
            EnsureDirectoryForAssetPath(ClipRoot);
            EnsureDirectoryForAssetPath(MetadataRoot);
            AssetDatabase.Refresh();
        }
        ...
        private static void EnsureDirectoryForAssetPath(string assetPath)
        {
            ...
            var packageRoot = packageInfo != null ? packageInfo.resolvedPath : Path.Combine(Directory.GetCurrentDirectory(), "AssetPacks", "BrassworksBreach.EnemyAnimationProxySet01");
